...
// 스팟 표는 프론트와 공유한다 — 좌표·특보구역이 두 곳에 따로 있으면 한쪽만 고쳤을 때
// 에러 없이 조용히 다른 숫자가 나오기 때문이다(예: sea 좌표가 어긋나면 홈 지도와 예보
// 화면이 서로 다른 파고를 보여준다). 표의 정의와 특보구역 조사 내용은 그 파일에 있다.
//
// import 방향이 낯설어 보이지만(서버가 정적 에셋 폴더를 참조) 선택이 아니라 제약이다 —
// 브라우저가 import 할 수 있으려면 파일이 public/ 안에 있어야 한다. wrangler(esbuild)가
// 번들할 때 이 데이터를 워커 코드에 그대로 인라인하므로 런타임 비용은 없다.
import { SPOTS as SPOT_LIST } from "../public/js/spots.js";

// 프론트는 배열(순서가 곧 표시 순서)로, 워커는 id 로 바로 찾는 형태가 편해서
// 여기서만 한 번 변환한다. 아래 코드는 전부 기존과 같은 SPOTS[id].lat 형태로 쓴다.
const SPOTS = Object.fromEntries(SPOT_LIST.map(s => [s.id, s]));
const SPOT_IDS = SPOT_LIST.map(s => s.id); // 파도 제보의 스팟 검증에도 그대로 재사용
